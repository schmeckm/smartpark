...
const oeeGaugeRef = ref<HTMLDivElement | null>(null)
let oeeGaugeChart: echarts.ECharts | null = null
...
const isWindowValid = computed(() => {
  if (!queryFromIso.value || !queryToIso.value) return false
  const from = new Date(queryFromIso.value)
  const to = new Date(queryToIso.value)
...
const availabilityBreakdown = computed(() => {
  const s = summary.value
  if (!s || s.windowMinutes <= 0) return null
  const wm = s.windowMinutes
  const plannedPct = Math.min(100, (s.plannedDowntimeMinutes / wm) * 100)
  const unplannedPct = Math.min(100, (s.unplannedDowntimeMinutes / wm) * 100)
  return {
    plannedPct: Math.round(plannedPct * 100) / 100,
    unplannedPct: Math.round(unplannedPct * 100) / 100,
    plannedMin: s.plannedDowntimeMinutes,
    unplannedMin: s.unplannedDowntimeMinutes,
    windowMin: wm,
  }
})
...
function minutesInCurrentWindow(ev: AssetDowntimeEventRow): number {
  if (!isWindowValid.value) return 0
  const fromD = new Date(queryFromIso.value)
  const toD = new Date(queryToIso.value)
  return Math.round(clipDowntimeMs(ev.startedAt, ev.endedAt, fromD, toD, new Date()) / 60000)
}

function downloadOeeCsv() {
  if (!assetId.value || !isWindowValid.value) return
  ...
  const now = new Date()
  const rows: string[][] = [
    ['OEE — Verfügbarkeit & Stillstände (Export)'],
    ['Park', parkLabel],
...
function onChartsResize() {
  paretoChart?.resize()
  oeeGaugeChart?.resize()
}

function renderOeeGaugeChart() {
  const el = oeeGaugeRef.value
  if (!el) return
  if (!oeeGaugeChart) oeeGaugeChart = echarts.init(el, undefined, { renderer: 'canvas' })
  const s = summary.value
  if (!s) {
    oeeGaugeChart?.clear()
    return
  ...
  oeeGaugeChart.setOption(
    {
      backgroundColor: 'transparent',
      series: [buildStandardPercentGaugeSeries({ value: val })],
    },
    true
  )
  oeeGaugeChart.resize()
...
function renderParetoChart() {
  ...
  const opt: EChartsOption = {
    ...
    grid: { left: 52, right: 48, bottom: 92, top: 34 },
    xAxis: {
      type: 'category',
      data: categories,
...
async function removeIncidentRow(row: Incident) {
  ...
  const ok = await askConfirm({
    message: `Vorfall „${row.title}“ wirklich unwiderruflich löschen?`,
    confirmLabel: 'Ja',
...
async function submitEvent() {
  ...
  if (!isWindowValid.value) {
    push('Bitte ein gültiges Zeitfenster setzen (Von < Bis).', 'error')
    return
  }
  ...
  if (endedAt && new Date(endedAt).getTime() < new Date(startedAt).getTime()) {
    push('Ende darf nicht vor Start liegen.', 'error')
    return
  }
  busy.value = true
  try {
...
async function removeEvent(row: AssetDowntimeEventRow) {
  if (!canEdit.value || !assetId.value) return
  const ok = await askConfirm({
    message: 'Diesen Stillstand wirklich löschen?',
    confirmLabel: 'Ja',
    cancelLabel: 'Abbrechen',
    variant: 'danger',
  })
...
watch([assetId, queryFromIso, queryToIso], () => scheduleRefresh())
...
onMounted(async () => {
  globalThis.addEventListener('resize', onChartsResize)
...
onUnmounted(() => {
  globalThis.removeEventListener('resize', onChartsResize)
  paretoChart?.dispose()
  paretoChart = null
  oeeGaugeChart?.dispose()
  oeeGaugeChart = null
  if (refreshDebounceTimer) clearTimeout(refreshDebounceTimer)
})
</script>
...
<template>
  <div class="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6">
    <nav class="text-xs text-slate-500">
      ...
      <span class="text-slate-400">OEE — Verfügbarkeit &amp; Stillstände</span>
      <span class="mx-2">·</span>
      <RouterLink to="/simulator" class="text-brand-400 hover:underline">Attraction-OEE-Simulator (MQTT)</RouterLink>
    </nav>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <h1 class="font-display text-xl font-semibold text-white">OEE — Verfügbarkeit &amp; Stillstände</h1>
      <div class="flex shrink-0 gap-2">
        <button
          type="button"
          class="rounded-lg border border-slate-600 px-3 py-2 text-sm text-slate-200 hover:bg-slate-800 disabled:opacity-40"
          :disabled="busy || !assetId || !isWindowValid"
          title="Lädt Stillstände, Verfügbarkeit und Vorfälle für die aktuelle Auswahl neu."
          @click="refreshData"
        >
          Aktualisieren
        </button>
    ...
    <p class="text-sm text-slate-400">
      <strong class="font-medium text-slate-300">Live-OEE und Telemetrie</strong> laufen über Sparkplug/MQTT — im Betrieb vom Feld,
      zum Testen über den
      <RouterLink to="/simulator" class="text-brand-400 hover:underline">Attraction-OEE-Simulator</RouterLink>
      (gleiche Metrik-Schnittstelle).
      <strong class="font-medium text-slate-300"> Diese Seite</strong> wertet dagegen
      <strong class="text-slate-200">gebuchte Stillstände</strong> in der Datenbank aus (<span class="font-mono text-slate-500">asset_downtime_events</span>
      — Planung, Nacharbeit, Reporting). Der Attraktion-OEE-Simulator kann dieselben Stillstände bei laufendem Betrieb
      <strong class="text-slate-200">zusätzlich in diese Tabelle spiegeln</strong> (Backend-Standard: aktiv; nur wenn die Anlage in
      <span class="font-mono text-xs">park_assets</span>
      mit Slug z. B. <span class="font-mono text-xs">arthur</span> geführt wird — sonst fehlt die UUID und nichts landet in der DB).
      Live-Werte ohne DB weiterhin unter
      <RouterLink to="/realtime/live?mode=oee" class="text-brand-400 hover:underline">Live Signals · OEE cockpit</RouterLink>.
      Die Verfügbarkeit hier ist aus den Buchungen berechnet, nicht direkt aus dem MQTT-Strom.
      <strong class="text-slate-300">Vorfälle (Incidents)</strong> sind davon getrennt: keine Ausfallzeit für diese Kennzahl.
      ...
      Zeitfenster: <strong class="text-slate-300">Kalender- und Uhrzeit-Picker</strong>.
      Die Stillstands-Tabelle unten listet nur Schnittmengen mit „Von/Bis“ — Konflikte können außerhalb des sichtbaren Fensters liegen.
    </p>
    ...
    <div v-if="summary" class="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
      ...
      <p class="mt-1 max-w-3xl text-xs text-slate-500">
        Klassisches OEE = Verfügbarkeit × Leistung × Qualität. Hier kommt <strong class="font-medium text-slate-400">A</strong> aus den
        gebuchten Stillständen (siehe Einleitung: MQTT/Simulator liefert Live-Metriken, diese Ansicht wertet die DB-Buchungen).
        Leistung (P) und Qualität (Q) sind hier noch nicht angebunden — Platzhalter unten.
      </p>
      <div class="mt-4 grid gap-6 lg:grid-cols-2 lg:items-start">
        <div class="min-h-[220px]">
          <div ref="oeeGaugeRef" class="h-[220px] w-full" />
          <p class="mt-1 text-center text-[11px] text-slate-600">
            Skala 0–100&nbsp;%. Den Zielwert (Ride MDM ·
            <span class="font-mono text-slate-500">asset_targets</span>
            ) sehen Sie rechts unter „Ziel Verfügbarkeit“.
          </p>
        </div>
        <div class="space-y-4">
          <div>
            <h3 class="text-xs font-medium uppercase tracking-wide text-slate-500">OEE-Faktoren</h3>
            <ul class="mt-2 space-y-2 text-sm">
              <li
                class="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-emerald-900/40 bg-emerald-950/20 px-3 py-2"
              >
              ...
              <li class="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-800/80 bg-slate-900/40 px-3 py-2">
                <span class="text-slate-500">P — Leistung <span class="text-[11px] text-slate-600">(Idealzyklus / Durchsatz)</span></span>
                <span class="text-xs text-slate-600">nicht erfasst</span>
              </li>
              <li class="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-800/80 bg-slate-900/40 px-3 py-2">
                <span class="text-slate-500">Q — Qualität</span>
                <span class="text-xs text-slate-600">nicht erfasst</span>
              </li>
            </ul>
          </div>
          <dl class="grid gap-3 text-sm sm:grid-cols-2">
            <div>
              <dt class="text-xs text-slate-500">Ziel Verfügbarkeit</dt>
            ...
            <div v-if="availabilityBreakdown" class="sm:col-span-2">
              ...
              <dd class="mt-1 text-slate-300">
                geplant {{ availabilityBreakdown.plannedPct }}% ({{ availabilityBreakdown.plannedMin }} min) · ungeplant
                {{ availabilityBreakdown.unplannedPct }}% ({{ availabilityBreakdown.unplannedMin }} min)
              </dd>
            </div>
          </dl>
        </div>
      </div>
      <p class="mt-4 border-t border-slate-800/80 pt-3 text-xs text-slate-500">{{ summary.methodology }}</p>
    </div>
    ...
    <div class="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
      ...
      <div ref="paretoChartRef" class="mt-4 h-[260px] w-full min-h-[200px]" />
    </div>
    ...
    <div class="space-y-2">
      <div>
        <h2 class="text-sm font-medium text-white">Stillstände — Erfassung</h2>
        <p class="mt-1 text-xs text-slate-500">
          Geplante und ungeplante Ausfallzeiten (<span class="font-mono text-slate-600">asset_downtime_events</span>); diese Einträge wirken auf die Verfügbarkeit und das OEE-Gauge oben.
        </p>
      ...
      <table class="min-w-full divide-y divide-slate-800 text-left text-sm">
        <thead class="bg-slate-950/80 text-xs uppercase text-slate-500">
          <tr>
            ...
            <th class="px-3 py-2">Dauer (min, Fenster)</th>
        ...
        <tbody class="divide-y divide-slate-800/80">
          ...
          >
            ...
            <td class="px-3 py-2 text-xs text-slate-200">{{ minutesInCurrentWindow(ev) }}</td>
          ...
          <tr v-if="!events.length">
            <td colspan="7" class="px-3 py-6 text-center text-slate-500">Keine Einträge im Fenster.</td>
          </tr>
        </tbody>
      </table>
      </div>
