...
module.exports = {
  async up(queryInterface) {
    // Idempotent: if a previous run inserted data but did not record completion in SequelizeData,
    // re-runs would duplicate primary keys and crash the API entrypoint. Skip when seed data exists.
    const [existingZone] = await queryInterface.sequelize.query(
      'SELECT 1 AS x FROM "zones" WHERE "id" = $1 LIMIT 1',
      { bind: [Z_PLAZA] }
    );
    if (existingZone && existingZone.length) {
      return;
    }
      ...
      [
        {
          ...
          name: 'Kingdom Plaza',
          type: 'MAIN_GATE',
          current_crowd_level: 0,
          forecast_crowd_level: 0,
          max_capacity: 9000,
          status: 'ACTIVE',
          adjacent_zone_ids: JSON.stringify([Z_ALPINE, Z_FAIRYTALE]),
          created_at: now,
          updated_at: now,
        },
        {
          ...
          max_capacity: 8500,
          status: 'ACTIVE',
          adjacent_zone_ids: JSON.stringify([
            Z_PLAZA,
            Z_MED,
            Z_NORDIC,
            Z_FAIRYTALE,
        ...
        },
        {
          ...
          max_capacity: 7000,
          status: 'ACTIVE',
          adjacent_zone_ids: JSON.stringify([Z_ALPINE, Z_NORDIC]),
          created_at: now,
          updated_at: now,
        },
        {
      ...
      [
        {
          ...
          priority: 'HIGH',
          message:
            'Alpine Summit crowd pressure elevated; pre-position security near queue merges.',
          status: 'OPEN',
          created_at: now,
          updated_at: now,
        },
        {
          ...
          priority: 'HIGH',
          message:
            'Silver Comet wait exceeds 45 minutes; route guests to Nordic Harbor family rides.',
          status: 'OPEN',
          created_at: now,
          updated_at: now,
        },
        {
          ...
          priority: 'MEDIUM',
          message:
            'Lagoon Splash is in maintenance; communicate alternate water play in Nordic Harbor.',
          status: 'ACCEPTED',
          created_at: now,
          updated_at: now,
        },
