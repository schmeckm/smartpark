...
class StaffService {
  ...
  listStaff() {
    return this.staffRepository.findAll({ includeZone: true });
  }

  async getStaffById(id) {
    const staff = await this.staffRepository.findById(id, { includeZone: true });
  ...
  async createStaff(payload) {
    if (payload.currentZoneId) await this.ensureZoneExists(payload.currentZoneId);
    ...
    const member = await this.staffRepository.findById(created.id, { includeZone: true });
  ...
  async updateStaff(id, payload) {
    ...
    const patch = {
      ...
      ...(payload.currentZoneId !== undefined ? { currentZoneId: payload.currentZoneId } : {}),
    ...
    if (payload.currentZoneId && payload.currentZoneId !== existing.currentZoneId) {
      await this.emitZoneRefresh(payload.currentZoneId);
    }
    const member = await this.staffRepository.findById(id, { includeZone: true });
    await auditLogService.log({
