...
## Phase 0 – umgesetzt (Ist-Stand Code)

**Park-Kontext**

- API-Client sendet bei gesetztem aktiven Park den Header `X-Park-Id` (synchron mit Pinia `parkContext` + `localStorage` `sp_active_park_id`).
- Nach Login lädt `MainLayout` die Parkliste (`GET /api/v1/parks`), setzt Standardpark (gespeicherter ID falls gültig, sonst erster Park) und zeigt einen **Park-Selector** in der Kopfzeile (wenn `rides.read`).
- Backend: Middleware `attachParkContext` nach `authenticate` — validiert UUID, Park existiert, Nutzer hat `rides.read` oder `ops.read`; setzt `req.parkContext`. `GET /api/v1/assets`: fehlender `parkId`-Query wird aus `X-Park-Id` befüllt; Konflikt Query ≠ Header → 400.

**RBAC**

- Neue Permissions: `ops.read`, `incidents.read`, `incidents.create`, `incidents.assign` (Server `src/constants/rbac.js` und Client `admin-dashboard/src/constants/rbac.ts` synchron). Incident-Routen folgen in Phase 1.

**Design-Basis**

- Composable `usePageSurfaces()` (theme-abhängige Oberflächenklassen); `SettingsView` nutzt es als Referenz für weitere Views.

**Nächste Ausbaustufe (nicht Phase 0)**

- `user_parks` / Mandant: harte Park-Zuordnung pro User statt „alle Parks mit rides.read“.
- Weitere Controller um `req.parkContext` ergänzen (Zonen, Events, …).